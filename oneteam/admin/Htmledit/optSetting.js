...
function Gi_getFileOpt(nRes,upLoadType,getPath,getTName){//选择上传和库
	 if(nRes=='enterUpLoad'){
		rRes = showModalDialog(''+getPath+'OptIframeUpLoad.asp?UPTName='+getTName+'&UPType='+upLoadType+'', null, 'dialogWidth: 305px; dialogHeight: 220px; center: yes; resizable: no; scroll: no; status: no;');
		if (rRes=='enterUpLib'){
			Gi_getFileOpt(rRes,upLoadType,getPath,getTName);
		}
	}
	else if(nRes=='enterUpLib'){
		rRes = showModalDialog(''+getPath+'OptIframeUpLib.asp?UPTName='+getTName+'&UPType='+upLoadType+'', null, 'dialogWidth: 800px; dialogHeight: 455px; center: yes; resizable: no; scroll: no; status: no;');
		if (rRes=='enterUpLoad'){
			Gi_getFileOpt(rRes,upLoadType,getPath,getTName);
		}
	}
