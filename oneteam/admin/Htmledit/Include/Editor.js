...
function document.onreadystatechange(){
	...
	if (!config.License){
		try{
			eWebEditor_License.innerHTML = "&copy; <a href='http://www.hotsales.net' target='_blank'><font color=#000000>Hotsales.net</font></a>";
		}
		catch(e){
		}
	}
...
function AttachSubmit() { 
	...
	if (!oForm) return;
	var html = getHTML();
	html=html.replace(/\<FONT face=Verdana\>([\s\S]*?)\<\/FONT\>/ig,"$1");
