...
<form method=POST action="info_check.asp" name="AEFContent" onSubmit="JScript:return Gi_validForm('AEFContent','Title','请输入<% = Menu %>标题！');">
  <table width="98%" border="0" cellspacing="2" cellpadding="3" align="center" class="Bgtbtop">
    ...
    <tr> 
      ...
      <td width="100" align="right" class="Inputbtn">排序：</td>
      <td class="Bgtdmain"><% = Is_Out(State_OnOff,"list")%>      <input  name="pxid" class="InputTxt" id="pxid" value="<% =pxid %>" size="10" maxlength="4">
数字大的显示在前面 </tr>
    <tr>
      <td align="right" class="Inputbtn"><input title="规格：184×254像素;" <%=IsUpImage%> name="UpImage3" type="button" class="InputBtn" id="UpImage3" onFocus="blur()" onClick="Gi_getImageUpLoad('AEFContent','UpLoad_Image','../HtmlEdit/','缩略图','delImageTag','delImage','resetImage','prevImage');" value="缩略图" style="cursor:pointer;background-color:#990000;color:#FFFFFF;" /></td>
...
%></td>
      ...
      <td align="right" class="Inputbtn"><input title="规格：宽度<600像素" <%=IsUpImage%> name="UpImage2" type="button" class="InputBtn" id="UpImage1" onFocus="blur()" onClick="Gi_getImageUpLoad('AEFContent','UpLoad_Image1','../HtmlEdit/','详细图片','delImageTag1','delImage1','resetImage1','prevImage1');" value="详细图片" style="cursor:pointer;background-color:#990000;color:#FFFFFF;" /></td>
      <td class="Bgtdmain"><table width="300" border="0" cellspacing="0" cellpadding="0">
        <tr>
